...
| 191 | **BUG (WORSENED in v3.41)**: swarmwrap.sty — figure dimension distortion at page boundaries. In v3.39, 4 of 50 figures (8%) had incorrect dimensions. In v3.41, **6 of 50 figures (12%)** are distorted — a 50% increase. ALL 6 are the LAST figure on their page and ALL extend to y=720.5 (page text area bottom). (1) Fig 10 (pg3): 88.5x118.0pt, expected 85.0x113.4pt. (2) Fig 14 (pg4): 95.7x63.8pt, expected 85.0x56.7pt. (3) Fig 25 (pg8): 77.3x77.3pt, expected 56.7x56.7pt. (4) Fig 32 (pg11): 69.6x104.4pt, expected 56.7x113.4pt. (5) Fig 36 (pg12): 73.3x36.7pt, expected 56.7x28.3pt. (6) Fig 43 (pg14): 121.2x90.9pt, expected 113.4x85.0pt. The 2 NEW distorted figures (Fig 32, Fig 36) are caused by v3.41's page reorganization (14→16 pages) creating 2 new page-bottom figure placements. **QA T101 ADDITIONAL FINDING:** All 6 distorted figures also have caption font size anomalies — the caption inside/below the distorted figure is rendered at the wrong size because the figure's minipage container is being stretched or squashed. Caption font sizes: Fig 10=9.33pt, Fig 14=10.09pt, Fig 25=12.23pt, Fig 32=11.01pt, Fig 36=5.80pt (35% smaller!), Fig 43=9.58pt. Normal caption size is 8.97pt. The 44 non-distorted figures ALL have exactly 8.97pt captions. Root cause unchanged: figures anchored to y=720.5 have distorted dimensions. Right-edge alignment remains perfect (all 50 at x1=476.5pt). **QA T107 1000-FIGURE SCALE TEST:** Compiled test-1000fig.pdf (1000 figures, US Letter, uniform 3cm×2cm). **91 of 1000 figures (9.1%) distorted** — ALL are the 6th (last) figure on pages with exactly 6 figures. Pattern: 91 pages have 6 figs (6th always distorted at 101.6×67.8pt instead of 85.0×56.7pt — 19.6% larger in both dims), 90 pages have 5 figs (5th always normal), 81 pages have 0 figs (parshape leak pages). Caption font size on distorted figures: 11.9pt (normal: ~10pt). The distortion is NOT at the page bottom (y=672.4, well within text area) — the common factor is being the LAST figure when the page is "full" (6 figs). Right-edge remains perfect (all 1000 at x1=477.5pt). **Root cause hypothesis refined:** NOT page-bottom anchoring — it's triggered when swarmwrap places the last figure on a "full" page (max figures that fit). The 50-fig test showed the same bug at y=720.5 because on A4 with varying figure sizes, the "last figure on full page" coincidentally ended at the text area bottom. ⛔ PROGRAMMER LOCKED — swarmwrap.sty only. | Programmer | pending | 2026-06-14 |
...
| 196 | **NEW BUG (v3.41, 1000-figure scale test)**: swarmwrap.sty — last figure on every 6-figure page rendered at wrong dimensions. In `test-1000fig.pdf` (1000 figures, all specified as 3cm×2cm via `\rule{\linewidth}{2cm}` inside `\begin{minipage}[t]{3cm}`), 91 of 1000 figures (9.1%) are distorted. ALL 91 are the LAST figure on their page, and ALL 6-figure pages (91 pages total) have this issue — 100% rate. **Distorted dimensions:** 101.6×67.8pt instead of expected 85.0×56.7pt (19.5% wider, 19.6% taller). The figure right edge is correct (477.5pt, same as normal figures), but the left edge shifts from 392.4pt to 375.8pt (16.6pt further left), making the figure AND its minipage container wider. **This is DISTINCT from Task #191** (which involves figures at y=720.5 page bottom being squashed). Here the distorted figures are at y1=672.4, well above the page bottom. The distortion is CONSISTENT (always exactly 101.6×67.8pt). **Pattern:** Pages alternate between 6-figure pages (with 1 distorted last fig), 5-figure pages (all normal), and 0-figure parshape-leak continuation pages (1 narrow line). First 5 figures on 6-figure pages are always normal. Only the 6th is distorted. **Possible cause:** When 6 figures are stacked on one page, the 6th figure's minipage width calculation may be affected by accumulated parshape/everypar state, or the deferred-NEWPAGE mechanism may be widening the figure zone for the last figure before a page break. ⛔ PROGRAMMER LOCKED — swarmwrap.sty only. | Programmer | pending | 2026-06-15 |
...
| 194 | **REGRESSION (v3.41)**: swarmwrap.sty — orphan lines on near-empty pages from deferred-NEWPAGE path. In test-stress-50.pdf (v3.41), 2 new near-empty pages exist that were NOT present in v3.39: (1) Page 6: 1 line ("elit. Etiam congue neque id dolor.") at width 163.5pt (narrow, parshape still active), 1.8% of page height. (2) Page 10: 1 line ("ligula.") at width 29.1pt (extremely narrow, parshape still active), 1.8% of page height. Both are orphan fragments of paragraphs that were being typeset in narrow parshape mode when the v3.41 deferred-NEWPAGE fired. When the figure gets deferred to the next page via `\newpage`, the paragraph's last line fragment is left on the current page. The parshape is NOT cleared before the `\newpage`, so the orphan line retains narrow width. **Root cause:** The deferred-NEWPAGE mechanism (v3.41's `\ifswarmwrap@placed` guard + `\swarmwrap@eff@total` overfull check) fires a `\newpage` but does not (a) finish the current paragraph with full-width parshape first, or (b) suppress the orphan line. **Fix approaches:** (1) Before firing the deferred NEWPAGE, insert `\parshape=1 72pt 451.28pt` (full-width) and let TeX re-break the remaining paragraph lines at full width before the page break. (2) Use `\vadjust` or `\vsplit` to move the orphan line to the next page. (3) Track the deferred-NEWPAGE event in `pre_shipout_filter` and if the shipped page has only 1-2 narrow lines, move them to the next page (complex). ⛔ PROGRAMMER LOCKED — swarmwrap.sty only. | Programmer | pending | 2026-06-14 |
