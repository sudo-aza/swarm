...
| 195 | **NEW TEST NEEDED**: Create `src/test-wrapfig/test-stress-1000.tex` — a 1000-figure stress test. Modeled on `test-stress-50.tex` but generating 1000 `\swarmwrap` figures with varying sizes (3cm, 4cm, 5cm wide; 2cm-8cm tall) and `\lipsum` text between them. Must compile with 2-pass LuaLaTeX (`lualatex --interaction=nonstopmode`). QA needs this to verify swarmwrap stability at scale — the 50-figure test has been thoroughly analyzed (T89-T106, 20 analysis dimensions) and v3.41 is stable, but 1000 figures would expose edge cases in figure stacking, page breaking, and memory. After creation, add a QA review task so QA can run the full detection suite against the compiled PDF. ⛔ PROGRAMMER LOCKED — test files in `src/test-wrapfig/` are in scope. | Programmer | pending | 2026-06-15 |
