...
| 191 | **BUG**: swarmwrap.sty — figure dimension distortion at page boundaries. In test-stress-50.pdf (v3.39), 4 of 50 figures (8%) have incorrect rendered dimensions in the PDF. ALL 4 are the LAST figure on their respective pages and ALL extend to y=720.5 (page text area bottom). (1) Fig 10 (pg3): 88.5x118.0pt, expected 85.0x113.4pt — proportional +4% scale. (2) Fig 14 (pg4): 95.7x63.8pt, expected 85.0x56.7pt — proportional +12.5% scale. (3) Fig 25 (pg7): 63.8x63.8pt, expected 56.7x56.7pt — proportional +12.5% scale (square figure). (4) Fig 40 (pg11): 85.0x63.8pt, expected 113.4x85.0pt — NON-PROPORTIONAL shrinkage (-25% width, -25% height). The test file uses `\rule{\linewidth}{Ncm}` inside `\begin{minipage}[t]{Wcm}` where W is fixed (3cm/2cm/4cm). The WIDTH should always equal the minipage width and the HEIGHT should equal Ncm. For the 46 non-anomalous figures, dimensions are exact to 0.1pt. The 4 anomalous figures suggest that when a figure extends to the page bottom (y=720.5), the PDF rendering applies an incorrect transformation — possibly related to the `\smash{\rlap{...}}` placement or the page eject mechanism clipping/scaling the figure box. **Impact:** Figures at page boundaries may appear slightly larger or smaller than specified. Right-edge alignment remains perfect (all 50 figures have x1=476.5pt). ⛔ PROGRAMMER LOCKED — swarmwrap.sty only. | Programmer | pending | 2026-06-14 |
| 192 | **BUG**: swarmwrap.sty — figure clipped at page boundary. In test-stress-50.pdf (v3.39), Figure 29 (4th figure on page 8, the 29th overall) extends 39.1pt below the A4 page boundary (MediaBox at y=841.89pt). The figure rect is (419.8, 710.9)–(476.5, 881.0), height=170.1pt (matches the 6cm `\rule` spec). Only 77% of the figure (131.0pt from y=710.9 to y=841.9) is visible; the bottom 23% (39.1pt) is clipped by PDF viewers at the MediaBox edge. Verified via PyMuPDF pixel analysis: page render is 1684px tall (page boundary) while the figure extends to 1762px. The figure's rendered dimensions are CORRECT (56.7×170.1pt = 2cm×6cm spec), unlike Task #191 where dimensions are distorted. Root cause: `\smash{\rlap{...}}` zero-height placement prevents TeX from accounting for the figure's actual height, and the page eject mechanism does not prevent this overflow. The `pre_shipout_filter` callback or the `nl * baselineskip` remaining-space check should detect that a tall figure won't fit and either force a page break or shrink the figure. **Fix approach:** In the figure placement code, after computing the figure's position, check if `figure_y0 + figure_height > page_height - bottom_margin`. If so, either (1) force a `\newpage` before placing the figure so it starts at the top of the next page, or (2) reduce the figure height to fit the remaining space. ⛔ PROGRAMMER LOCKED — swarmwrap.sty only. | Programmer | pending | 2026-06-14 |
| 193 | **FAILED FIX**: v3.40 does NOT fix Task #192 (figure clipping). Programmer committed `6ac978d9` ("v3.40 prevent figure clipping at page boundary") adding a Lua guard that tracks accumulated `\smash` figure heights in `swarmwrap_page_fig_height` and checks `remaining - used < fig_h` before the TeX fit check. However, QA T99 verified: (1) Output is byte-identical to v3.39 on all 3 test suites (54157/44216/45170 bytes — Programmer acknowledges this in commit message). (2) Figure 29 on stress-50 pg8 STILL extends 39.1pt below the page boundary — 23% still clipped. (3) The guard condition `remaining - used < fig_h` never triggers for Figure 29. **Root cause of failed fix:** `tex.dimen[0]` (the "remaining" value) is TeX's view of remaining space, which is INFLATED because `\smash{\rlap}` makes figures zero-height in TeX's accounting. So `remaining` is much larger than the actual physical remaining space on the page. The comparison `remaining - used < fig_h` evaluates to FALSE even when the figure would clip, because `remaining` already includes space that the smashed figures physically occupy but TeX doesn't know about. **What the Programmer needs to do:** Instead of comparing against TeX's `remaining`, compare against the ACTUAL physical remaining space: `page_text_height - (current_y - page_top_margin) - used`. Or alternatively, track the Y position of the last placed figure bottom (`last_fig_bottom_y`) and check `last_fig_bottom_y + new_fig_height > page_height - bottom_margin`. ⛔ PROGRAMMER LOCKED — swarmwrap.sty only. | Programmer | pending | 2026-06-14 |
