...
| 190 | **BUG (REGRESSION)**: swarmwrap.sty v3.39 — parshape leak count INCREASED in test-pagebreak-variations.pdf from 28 (v3.37) to 40 (v3.39), a 43% regression. 5 leaked pages: pg2 (15 lines, SEVERE), pg5 (2 lines, MILD), pg9 (24 lines, SEVERE), pg11 (12 lines, MODERATE), pg13 (11 lines, MODERATE). **Root cause hypothesis (QA T93):** The figure stack pop in `post_linebreak_filter` restores `remaining@nl` and `everypar` from a previous figure's saved state. In pbv's test pattern (1 figure + 1 lipsum paragraph + `\newpage` per scenario), the stack pop can restore narrowing state from a figure on the PREVIOUS page (which was already shipped and cleared by `pre_shipout_filter`). The restored `everypar` then narrows text on the new page even though no figure is present. The `pre_shipout_filter` clears state on page ship, but if the stack pop restores state BEFORE the page ships (during paragraph breaking), the narrow lines are already formatted and leak to the next page. **Stress-50 is unaffected** (0 leaks, down from 44) because figures are densely packed and the stack always has current-page figures. **Fix approach:** In the stack pop code in `post_linebreak_filter`, check whether the popped entry's figure is still on the CURRENT page (e.g., by comparing a page counter or checking if `swarmwrap_page_shipped` was set since the push). If the figure is from a previous page, discard the pop and clear everypar instead of restoring the stale state. Alternatively, clear the fig_stack in `\swarmwrapnext` at the start of each new figure placement if `swarmwrap_page_shipped` is true. ⛔ PROGRAMMER LOCKED — swarmwrap.sty only. | Programmer | pending | 2026-06-14 |
...
> - pbv: 40 leaks on 5 pages (was 28 on 5 pages). **43% REGRESSION.** ⚠️ Task #190 created.
>
