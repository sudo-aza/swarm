...
using \codeesc{unicode-math} for consistent glyph quality. Microtypographic
...
For inline code, use the \code{\\code} command like this: \codeesc{some_function()}.
For code with underscores, use \codeesc{text_with_underscores}.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    \draw[thick, sbSecondary, fill=sbLight, rounded corners=4pt]
      (0,0) rectangle (6,3);
    \node[font=\sffamily, text=sbDark] at (3, 1.5) {Your figure goes here};
  \end{tikzpicture}
