...
--
-- LAYER 2 (v3.70): Transition penalty at narrow->full boundary.
-- After TeX breaks a paragraph into lines (hlists), find the LAST narrow
-- line (narrow->full transition point) and insert a penalty there.
-- v3.70 original design: penalty strength was CONDITIONAL on remaining page
-- space (computed from \swarmwrap@remaining). If narrow zone exceeded remaining:
--   - Use -10000 (FORCED break, TeX cannot override).
--   - Otherwise: use -5000 (normal encouragement).
-- v3.75 REMOVED \swarmwrap@remaining, so the conditional -10000 path never
-- triggers. The penalty is now always -5000 (encouragement only). DEFER
-- margin is the primary ghost-narrowing prevention mechanism.
-- (Fires only once per unique paragraph shape due to LuaTeX caching.)
--
...
function swarmwrap_post_lb(head, groupcode)
  ...
  -- v3.70 (unconditional since v3.75): Penalty at narrow->full transition.
  -- v3.75 removed \swarmwrap@remaining (the remaining-space dimen set in
  -- \swarmwrapnext from pagegoal-pagetotal). Without it, the conditional
  -- -10000 forced-break path never triggers. Penalty is always -5000.
  local penalty_value = -5000  -- unconditional encouragement (v3.75+)
  local bs = tex.skip["baselineskip"].width
  if bs > 0 then
    local ok, err = pcall(function()
...
texio.write_nl("swarmwrap: callback v4.02 loaded (needspace + transition-penalty + caption-zone measurement)")
